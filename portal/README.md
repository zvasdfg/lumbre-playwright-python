...
Worker request handlers never write to the filesystem. Production currently
serves the bundled registry read-only; authenticated hosted writes remain a
...
result is still a read-only public-demo candidate; no deployment is performed
or represented as complete by this repository.
